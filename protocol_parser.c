...
#include "../includes/linux/olsr_header.h"
#include "../includes/linux/batman_header.h"
#include "../includes/linux/batman_adv_header-14.h"
#include "util.h"
#include "packet_types.h"

struct uwifi_packet_info;

extern int uwifi_parse_raw(unsigned char* buf, size_t len, struct uwifi_packet_info* p);
static int parse_llc(unsigned char* buf, size_t len, struct uwifi_packet_info* p);
static int parse_ip_header(unsigned char* buf, size_t len, struct uwifi_packet_info* p);
static int parse_udp_header(unsigned char* buf, size_t len, struct uwifi_packet_info* p);
static int parse_olsr_packet(unsigned char* buf, size_t len, struct uwifi_packet_info* p);
static int parse_batman_packet(unsigned char* buf, size_t len, struct uwifi_packet_info* p);
static int parse_batman_adv_packet(unsigned char* buf, size_t len, struct uwifi_packet_info* p);
static int parse_meshcruzer_packet(unsigned char* buf, size_t len, struct uwifi_packet_info* p, int port);

/* return true if we parsed enough = min ieee header */
bool parse_packet(unsigned char* buf, size_t len, struct uwifi_packet_info* p)
{
	int ret = uwifi_parse_raw(buf, len, p);
	if (ret == 0)
		return true;
	else if (ret < 0)
		return false;
...
static int parse_llc(unsigned char* buf, size_t len, struct uwifi_packet_info* p)
{
	...
	if (ntohs(*((uint16_t*)buf)) == 0x4305) {
		...
		printlog(LOG_DEBUG, "* parse LLC left %zd\n", len - 8);
		return 8;
	}
}
...
static int parse_batman_adv_packet(unsigned char* buf, size_t len, struct uwifi_packet_info* p)
{
	...
	printlog(LOG_DEBUG, "parse bat len %zd type %d vers %d\n", len, bp->packet_type, bp->version);
	...
	if (bp->version == 14) {
		...
		case BAT_OGM:
			...
			if (bp->gw_flags != 0 && memcmp(bp->orig, p->wlan_src, MAC_LEN) == 0)
				p->bat_gw = 1;
			printlog(LOG_DEBUG, "OGM %d %d\n", bp->gw_flags, p->bat_gw);
			return 0;
		case BAT_ICMP:
			printlog(LOG_DEBUG, "ICMP\n");
			break;
		case BAT_UNICAST:
			printlog(LOG_DEBUG, "UNI %zu\n", sizeof(struct unicast_packet));
			return sizeof(struct unicast_packet) + 14;
		case BAT_BCAST:
			printlog(LOG_DEBUG, "BCAST\n");
			break;
		case BAT_VIS:
...
static int parse_udp_header(unsigned char* buf, size_t len, struct uwifi_packet_info* p)
{
	...
	printlog(LOG_DEBUG, "UPD dest port: %d\n", ntohs(uh->uh_dport));
...
static int parse_olsr_packet(unsigned char* buf, size_t len, struct uwifi_packet_info* p)
{
	...
	printlog(LOG_DEBUG, "OLSR msgtype: %d\n*** ", msgtype);
...
static int parse_batman_packet(__attribute__((unused)) unsigned char* buf,
			       ...
			       __attribute__((unused)) struct uwifi_packet_info* p)
{
	p->pkt_types |= PKT_TYPE_BATMAN;
	return 0;
}
